---
title: daisyUI 5 Reference
description: Complete daisyUI 5 component reference and usage guide
---
...
4. A CSS file with Tailwind CSS and daisyUI looks like this (if it's a node dependency)
```css
@import "tailwindcss";
@plugin "daisyui";
```
...
9. don't add `bg-base-100 text-base-content` to body unless it's necessary
10. For design decisions, use Refactoring UI book best practices
...
- you need to add `aria-live="polite"` and `aria-label` with the number value so screen readers can properly read changes
...
<div class="hover-3d my-12 mx-2">
  <figure class="max-w-100 rounded-2xl">
    <img src="" alt="Tailwind CSS 3D card" />
  </figure>
  <div></div>
  <div></div>
  <div></div>
...
- you need to add `aria-valuenow` attribute with the current value and `role="progressbar"` so screen readers can properly read the value and identify it as a progress element
